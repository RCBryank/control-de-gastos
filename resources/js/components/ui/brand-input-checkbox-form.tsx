...
interface BrandCheckbox extends React.ComponentProps<"input"> {
    propdivisChecked: boolean,
    customOnChangeEvent: Function
}

export default function BrandInputCheckboxForm({ children, propdivisChecked, customOnChangeEvent, ...props }: BrandCheckbox) {
    const id = useId();
    ...
    return (
        <>
            <div>
                <input type="checkbox" id={id} className="hidden" checked={divchecked} onChange={() => { }}></input>
